...
public class ChildCareAdminContracts extends ChildCareBlock {
    ...
    public void init(IWContext iwc) throws Exception {
        ...
        if (child != null) {
            ...
            add(this.getLocalizedHeader(
                    "child_care.no_child_or_application_found",
                    "No child or application found."));
        }
    }
    ...
    private void save(IWContext iwc) {
        ...
        try {
            employmentTypeID = Integer.parseInt(iwc
                    .getParameter(PARAM_EMPLOYMENT));
        } catch (NumberFormatException nfe) {
            employmentTypeID = -1;
        }

        IWTimestamp placementDate = new IWTimestamp(iwc
                .getParameter(PARAM_PLACEMENT_DATE));
        IWTimestamp terminationDate = null;
        if (iwc.isParameterSet(PARAM_TERMINATION_DATE)) {
            terminationDate = new IWTimestamp(iwc
                    .getParameter(PARAM_TERMINATION_DATE));
        }
        IWTimestamp replyDate = null;
        if (iwc.isParameterSet(PARAM_LAST_REPLY_DATE))
                replyDate = new IWTimestamp(iwc
                        .getParameter(PARAM_LAST_REPLY_DATE));
        String comment = iwc.getParameter(PARAM_COMMENT);
        String preSchool = iwc.getParameter(PARAM_PRE_SCHOOL);
        String extraContractMessage = iwc
                .getParameter(PARAM_EXTRA_CONTRACT_MESSAGE);
        String extraContractMessageOther = iwc
                .getParameter(PARAM_EXTRA_CONTRACT_OTHER_MESSAGE);
        boolean extraContract = BooleanInput.getBooleanReturnValue(iwc
                .getParameter(PARAM_EXTRA_CONTRACT));
        boolean extraContractOther = BooleanInput.getBooleanReturnValue(iwc
                .getParameter(PARAM_EXTRA_CONTRACT_OTHER));
        int groupID = -1;
        try {
        ...
        try {
            ...
            success = getBusiness().importChildToProvider(
                    getSession().getApplicationID(), getSession().getChildID(),
                    getSession().getChildCareID(), groupID, careTime,
                    employmentTypeID, schoolTypeID, comment, placementDate,
                    terminationDate, iwc.getCurrentLocale(), owner,
                    iwc.getCurrentUser(), true, replyDate, preSchool,
                    extraContract, extraContractMessage, extraContractOther,
                    extraContractMessageOther);
        } catch (RemoteException re) {
            success = false;
            errorMessage = localize("child_care.submit_failed",
                    "Submit failed.");
        } catch (AlreadyCreatedException ace) {
            success = false;
            errorMessage = localize("child_care.contract_already_exists",
                    "Active contract already exists for this child.");
        }
        ...
        if (success) {
            ...
            if (getResponsePage() != null) {
                ...
                add(getSmallHeader(localize("child_care.submit_successful",
                        "Submit successful.")));
                add(new Break(2));
    ...
    public int process(IWContext iwc) {
        try {
            if (getSession().getApplicationID() != -1) {
                application = getBusiness().getApplication(
                        getSession().getApplicationID());
                child = application.getChild();
                isUpdate = true;
                if (application.getApplicationStatus() == getBusiness()
                        .getStatusContract()) finalize = true;
            } else if (getSession().getChildID() != -1) {
                child = getBusiness().getUserBusiness().getUser(
                        getSession().getChildID());
            }
        } catch (RemoteException re) {
            child = null;
            log(re);
    ...
    public void showForm(IWContext iwc) {
        ...
        table.add(getSmallText(PersonalIDFormatter.format(
                child.getPersonalID(), iwc.getCurrentLocale())), 3, row++);
        ...
        try {
            if (child != null)
                    hasCheck = getCheckBusiness(iwc).hasGrantedCheck(child);
        } catch (RemoteException e) {
            e.printStackTrace();
        }

        table.add(getLocalizedLabel(LABEL_GRANTED_CHECK, "Granted check"), 1,
                row);
        if (!hasCheck)
            table.add(getSmallText(localize("child_care.no", "No")), 3, row++);
        else
            table
                    .add(getSmallText(localize("child_care.yes", "Yes")), 3,
                            row++);
        ...
        if (parents != null) {
            table.add(
                    getLocalizedHeader("child_care.custodians", "Custodians"),
                    1, row++);
            ...
            while (iter.hasNext()) {
                ...
                table.add(getLocalizedLabel(LABEL_USER_NAME, "Name"), 1, row);
                table
                        .add(getSmallText(parent.getNameLastFirst(true)), 3,
                                row++);
                table.add(getLocalizedLabel(LABEL_PERSONAL_ID, "Personal ID"),
                        1, row);
                table.add(getSmallText(PersonalIDFormatter.format(parent
                        .getPersonalID(), iwc.getCurrentLocale())), 3, row++);
                table.add(getLocalizedLabel(LABEL_GETS_BILL, "Gets bill"), 1,
                        row);
                RadioButton getBill = this.getRadioButton(PARAM_GETBILL, parent
                        .getPrimaryKey().toString());
                getBill.keepStatusOnAction(true);
                getBill.setMustBeSelected(localize(
                        "child_care.must_select_billed_to",
                        "You must select who to send the bill to."));
                if (application != null) {
        ...
        try {
            Collection schools = getSchoolBusiness(iwc).getSchoolHome()
                    .findAllByCategory(
                            getSchoolBusiness(iwc).getCategoryChildcare());
            SelectorUtility sel = new SelectorUtility();
            DropdownMenu prov = (DropdownMenu) sel.getSelectorFromIDOEntities(
                    new DropdownMenu(getSession().getParameterChildCareID()),
                    schools, "getName");
            prov.addMenuElementFirst("-1", "");
            prov.setToSubmit(true);
            prov.setAsNotEmpty(localize("child_care.must_select_provider",
                    "You must select a provider."));
            if (application != null)
                    prov.setSelectedElement(application.getProviderId());
            if (getSession().getChildCareID() != -1)
                    prov.setSelectedElement(getSession().getChildCareID());

            table.add(getStyledInterface(prov), 3, row);

            GenericButton createProvider = getButton(new GenericButton("",
                    localize("child_care.create_provider", "Create provider")));
            createProvider
                    .setWindowToOpen(CentralPlacementProviderEditor.class);
        ...
        table.add(getLocalizedLabel(LABEL_PROVIDER_OPERATION, "Operation"), 1,
                row);
        try {
            Collection school_types = getSchoolBusiness(iwc)
                    .getSchoolTypeHome().findAllByCategory(
                            getSchoolBusiness(iwc).getCategoryChildcare()
                                    .getCategory());
            SelectorUtility sel = new SelectorUtility();
            DropdownMenu op = (DropdownMenu) sel.getSelectorFromIDOEntities(
                    new DropdownMenu(PARAM_OPERATION), school_types, "getName");
        ...
        try {
            Collection school_classes = getSchoolBusiness(iwc)
                    .getSchoolClassHome().findBySchoolAndCategory(
                            getSession().getChildCareID(),
                            getSchoolBusiness(iwc).getCategoryChildcare()
                                    .getCategory());
            SelectorUtility sel = new SelectorUtility();
            DropdownMenu op = (DropdownMenu) sel.getSelectorFromIDOEntities(
                    new DropdownMenu(PARAM_GROUP), school_classes, "getName");
            if (!op.isEmpty()) {
                if (!isUpdate || finalize) {
                    op.setAsNotEmpty(localize("child_care.must_select_group",
                            "You must select a group."), "-1");
                }
            ...
            GenericButton createGroup = getButton(new GenericButton("",
                    localize("child_care.create_group", "Create group")));
            createGroup
                    .setWindowToOpen(CentralPlacementSchoolGroupEditor.class);
            table.add(Text.getNonBrakingSpace(), 3, row);
        ...
        table.add(getSmallHeader(localize("child_care.pre_school",
                "Specify pre-school:")), 1, row);
        TextInput preSchool = (TextInput) getStyledInterface(new TextInput(
                PARAM_PRE_SCHOOL));
        ...
        TextInput hoursWeek = (TextInput) getStyledInterface(new TextInput(
                PARAM_HOURS));
        hoursWeek.keepStatusOnAction(true);
        hoursWeek.setLength(2);
        if (!isUpdate || finalize) {
            hoursWeek.setAsNotEmpty(localize(
                    "child_care.child_care_time_required",
                    "You must fill in the child care time."));
            hoursWeek.setAsIntegers(localize(
                    "child_care.only_integers_allowed",
                    "Not a valid child care time."));
        }
        table.add(getLocalizedLabel(LABEL_HOURS, "Hours pr. week"), 1, row);
        table.add(hoursWeek, 3, row++);

        try {
            DropdownMenu employment = this.getEmploymentTypes(PARAM_EMPLOYMENT,
                    -1);
            employment.keepStatusOnAction(true);
            if (!isUpdate || finalize) {
                employment.setAsNotEmpty(localize(
                        "child_care.must_select_employment_type",
                        "You must select employment type."), "-1");
            }
            table
                    .add(getLocalizedLabel(LABEL_EMPLOYMENT, "Employment"), 1,
                            row);
        ...
        BooleanInput hasExtraContract = (BooleanInput) getStyledInterface(new BooleanInput(
                PARAM_EXTRA_CONTRACT));
        if (application != null)
                hasExtraContract.setSelected(application.getHasExtraContract());
        TextInput extraContractMessage = (TextInput) getStyledInterface(new TextInput(
                PARAM_EXTRA_CONTRACT_MESSAGE));
        if (application != null
                && application.getExtraContractMessage() != null)
                extraContractMessage.setContent(application
                        .getExtraContractMessage());
        table.add(getSmallHeader(localize(LABEL_EXTRA_CONTRACT,
                "Extra contract")), 1, row);
        table.add(hasExtraContract, 3, row);
        table.add(new Text(Text.NON_BREAKING_SPACE + Text.NON_BREAKING_SPACE
                + Text.NON_BREAKING_SPACE), 3, row);
        table.add(getSmallHeader(localize(LABEL_EXTRA_CONTRACT_MESSAGE,
                "Message")), 3, row);
        table.add(new Text(Text.NON_BREAKING_SPACE + Text.NON_BREAKING_SPACE),
                3, row);
        table.add(extraContractMessage, 3, row++);

        BooleanInput hasExtraContractOther = (BooleanInput) getStyledInterface(new BooleanInput(
                PARAM_EXTRA_CONTRACT_OTHER));
        if (application != null)
                hasExtraContractOther.setSelected(application
                        .getHasExtraContractOther());
        TextInput extraContractOtherMessage = (TextInput) getStyledInterface(new TextInput(
                PARAM_EXTRA_CONTRACT_OTHER_MESSAGE));
        if (application != null
                && application.getExtraContractMessageOther() != null)
                extraContractOtherMessage.setContent(application
                        .getExtraContractMessageOther());
        table.add(getSmallHeader(localize(LABEL_EXTRA_CONTRACT_OTHER,
                "Extra contract other")), 1, row);
        table.add(hasExtraContractOther, 3, row);
        table.add(new Text(Text.NON_BREAKING_SPACE + Text.NON_BREAKING_SPACE
                + Text.NON_BREAKING_SPACE), 3, row);
        table.add(getSmallHeader(localize(LABEL_EXTRA_CONTRACT_OTHER_MESSAGE,
                "Message")), 3, row);
        table.add(new Text(Text.NON_BREAKING_SPACE + Text.NON_BREAKING_SPACE),
                3, row);
        ...
        DateInput placementDate = (DateInput) getStyledInterface(new DateInput(
                PARAM_PLACEMENT_DATE));
        placementDate.keepStatusOnAction(true);
        placementDate.setToDisplayDayLast(true);
        placementDate.setAsNotEmpty(localize(
                "child_care.must_select_placement_date",
                "You have to select a placement date"));
        if (application != null)
                placementDate.setDate(application.getFromDate());
        table.add(getLocalizedLabel(LABEL_PLACEMENT_DATE, "Placement date"), 1,
                row);
        table.add(placementDate, 3, row++);

        DateInput terminationDate = (DateInput) getStyledInterface(new DateInput(
                PARAM_TERMINATION_DATE));
        terminationDate.keepStatusOnAction(true);
        terminationDate.setToDisplayDayLast(true);
        table.add(
                getLocalizedLabel(LABEL_TERMINATION_DATE, "Termination date"),
                1, row);
        table.add(terminationDate, 3, row++);

        if (application != null) {
            DateInput replyDate = (DateInput) getStyledInterface(new DateInput(
                    PARAM_LAST_REPLY_DATE));
            replyDate.keepStatusOnAction(true);
            replyDate.setToDisplayDayLast(true);
            replyDate.setAsNotEmpty(localize(
                    "child_care.must_select_last_reply_date",
                    "You have to select a last reply date"));
            table.add(getLocalizedLabel(LABEL_LAST_REPLY_DATE,
                    "Last reply date"), 1, row);
            table.add(replyDate, 3, row++);
        }

        TextArea comment = (TextArea) getStyledInterface(new TextArea(
                PARAM_COMMENT));
        ...
        SubmitButton submit = (SubmitButton) getButton(new SubmitButton(
                localize("child_care.save", "Save"), PARAM_SUBMIT, "true"));
        form.add(table);
        table.add(submit, 1, row);
        form.setToDisableOnSubmit(submit, true);

        if (getResponsePage() != null) {
            GenericButton cancel = getButton(new GenericButton("", localize(
    ...
    private CommuneUserBusiness getUserService(IWContext iwc)
    ...
    private CheckBusiness getCheckBusiness(IWContext iwc)
            throws RemoteException {
        return (CheckBusiness) IBOLookup.getServiceInstance(iwc,
                CheckBusiness.class);
    }
